...
//static const adc_atten_t atten = ADC_ATTEN_DB_11; // Valor de atenuação padrão
//static esp_adc_cal_characteristics_t *adc_chars;  // Estrutura de calibragem de ADC
...
void app_main(void)
{
    ...
    ESP_ERROR_CHECK(bh1750_init_desc(&dev, ADDR_BH1750, I2C_PORT, SDA_GPIO, SCL_GPIO));
    ESP_ERROR_CHECK(bh1750_setup(&dev, BH1750_MODE_CONTINUOUS, BH1750_RES_HIGH));
    ESP_ERROR_CHECK(bme680_init_desc(&sensor, ADDR_BME680, I2C_PORT, SDA_GPIO , SCL_GPIO));
    ...
    bme680_set_oversampling_rates(&sensor, BME680_OSR_4X, BME680_OSR_2X, BME680_OSR_2X); // BME680_OSR_4X, BME680_OSR_NONE, BME680_OSR_2X
    ...
    while (1)
    {
        ...
        if (dht_read_float_data(DHT_TYPE_AM2301, 3, &humidity, &temperature) != ESP_OK)
        ...
        if (bme680_force_measurement(&sensor) == ESP_OK)
        {
            ...
            if (bme680_get_results_float(&sensor, &values) == ESP_OK)
                printf("BME680 Sensor: %.2f °C, %.2f %%, %.2f hPa, %.2f Ohm, Lux: %d, DHT22 %.2lf °C, %.2f%% \n",
                        values.temperature, values.humidity, values.pressure, values.gas_resistance, lux, temperature, humidity);
        }
